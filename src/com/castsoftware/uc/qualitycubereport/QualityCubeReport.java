	...
	private void extractQualityCube() {
		...
		if (this.qualityRuleDetailLookup) {
			nameCells = new String[] { "Type", "Parent name", "Version", "Quality rule id", "Quality rule name", "Critical",
					"Technologies", "Standard", "Href", "Rest Href", };
		}	else {
		nameCells = new String[] { "Type", "Parent name", "Version", "Quality rule id", "Quality rule name", "Critical",
				"Technologies", "Href", "Rest Href", };
		}
		...
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				if (isQualityRuleDetailLookup() && qi.getRestHref() != null && qi.getRestHref().indexOf("null") < 0 && qi.getFullRestHref() != null) {
					responseBody =executeRestAPIRequest(qi.getFullRestHref(), null, null, "iso-8859-1", logger);
					// Parsing the JSON structure
				...
				if (qi.getHref() != null) {
					cell = row.createCell(iCell++);
					cell.setCellValue(qi.getHref());
					url.setAddress(URLEncoder.encode(qi.getHref(),"UTF-8"));
					cell.setHyperlink(url);
				}					
				if (qi.getFullRestHref() != null) {
					cell = row.createCell(iCell++);
					cell.setCellValue(qi.getFullRestHref());
					url.setAddress(URLEncoder.encode(qi.getFullRestHref(),"UTF-8"));
					cell.setHyperlink(url);					
				}
			}
			// list of extensions
				...
				for (int j = 0; j < array.length(); j++) {
					...
					for (int k = 0; k < arrayextVersion.length(); k++) {
						...
						for (int k1 = 0; k1 < arrayQRExtension.length(); k1++) {
							...
							if (isQualityRuleDetailLookup() && qiext.getRestHref() != null && qiext.getRestHref().indexOf("null") < 0 && qiext.getFullRestHref() != null) {
								responseBody =executeRestAPIRequest(qiext.getFullRestHref(), null, null, "iso-8859-1", logger);
								// Parsing the JSON structure
								JSONObject jsonObject = checkJSONObjectResponseBodyErrors(responseBody);
								JSONArray  jsonArrayBC = jsonObject.getJSONArray("businessCriteria");
								JSONArray  jsonArrayTC = jsonObject.getJSONArray("technicalCriteria");
							...
							if (qiext.getHref() != null) {
								cell = row.createCell(iCell++);
								cell.setCellValue(qiext.getHref());						
							}							
							if (qiext.getFullRestHref() != null) {
								cell = row.createCell(iCell++);
								cell.setCellValue(qiext.getFullRestHref());						
							}
						}
